...
import Filters from '@/components/tutorlisting/Filters';
import { useSession } from 'next-auth/react';
import { useRouter } from 'next/navigation';
import TutorCard from '@/components/tutorlisting/TutorCard';

const page1 = ({ data, recordsPerPage, currentPage }) => {
  ...
  const applyFiltersAndSort = () => {
    ...
    if (filters.query) {
      ...
      filtered = filtered.filter(request => 
        ...
        request.profile_tag_line?.toLowerCase().includes(searchTerm) ||
        request.skill_names?.toLowerCase().includes(searchTerm) || 
        request.city?.toLowerCase().includes(searchTerm) ||
        request.state?.toLowerCase().includes(searchTerm) ||
        request.language_preferences?.toLowerCase().includes(searchTerm) ||
        request.tutoring_preferences?.toLowerCase().includes(searchTerm)
      );
    }
     ...
     filtered = filtered.filter(request => {
       ...
       if (filters.tutoring_preferences && filters.tutoring_preferences.length > 0) {
         const selectedTypes = filters.tutoring_preferences.split(',');
         const requestTypes = request.tutoring_preferences ? request.tutoring_preferences.split(',') : [];
       ...
       if (filters.gender && request.gender !== filters.gender) return false;
       
       // Handle multiple languages
       if (filters.language_preferences && filters.language_preferences.length > 0) {
         const selectedLanguages = filters.language_preferences.split(',');
         const requestLanguages = request.language_preferences ? request.language_preferences.split(',') : [];
            ...
            <div className="col-xl-8 col-xxl-9">
              <div className="tu-tutors-list">
                {displayedRequests.map((request) => (
                  <TutorCard key={request.id} request={request} />
                ))}
